      ...
      fluidRow(
        column(
          ...
          box(
            ...
            solidHeader = TRUE,
            status = "info",
            withSpinner(plotlyOutput("degree_assort_scatter", height = "400px"), color = "#CC0000", type = 4)
          )
        ),
        column(
          width = 6,
          box(
            ...
            solidHeader = TRUE,
            status = "info",
            withSpinner(plotlyOutput("neighbor_degree_dist", height = "400px"), color = "#CC0000", type = 4)
          )
        )
      )
    ),
    ...
    tabPanel(
      ...
      fluidRow(
        box(
          ...
          status = "info",
          tabsetPanel(
            id = "attr_assort_viz_tabs", selected = "ggraph",
            tabPanel("ggraph",      value = "ggraph",      br(), withSpinner(plotOutput("attr_assort_ggraph",      height = "500px"), color = "#CC0000", type = 4)),
            tabPanel("Interactive", value = "interactive", br(), withSpinner(visNetworkOutput("attr_assort_plot", height = "500px"), color = "#CC0000", type = 4))
          )
        )
      ),
      ...
      fluidRow(
        box(
          ...
          solidHeader = TRUE,
          status = "info",
          withSpinner(plotlyOutput("attr_connection_bar", height = "400px"), color = "#CC0000", type = 4)
        )
      )
    ),
    ...
    tabPanel(
      ...
      fluidRow(
        box(
          ...
          solidHeader = TRUE,
          status = "info",
          withSpinner(plotlyOutput("mixing_matrix_heatmap", height = "500px"), color = "#CC0000", type = 4)
        )
      )
    ),
    ...
    tabPanel(
      ...
      fluidRow(
        box(
          ...
          solidHeader = TRUE,
          status = "info",
          withSpinner(plotlyOutput("connected_values_plot", height = "500px"), color = "#CC0000", type = 4)
        )
      )
    ),
