centrality_server <- function(input, output, session, rv) {
  ...
  output$degree_plot <- renderVisNetwork({
    ...
    vis_data$nodes$title <- paste0("<b>", vis_data$nodes$label, "</b><br/>Degree: ", round(scores, 3))
  ...
  output$degree_ggraph <- renderPlot({
    req(rv$centrality_results$degree, rv$igraph)
    scores <- rv$centrality_results$degree
    build_ggraph_plot(rv$igraph, input, node_size_scores = scores)
  }, res = 110)
  ...
  output$closeness_plot <- renderVisNetwork({
    ...
    vis_data$nodes$value <- scales::rescale(scores, to = c(input$node_size * 0.4, input$node_size * 3))
    vis_data$nodes$title <- paste0("<b>", vis_data$nodes$label, "</b><br/>Closeness: ", round(scores, 3))
  ...
  output$closeness_ggraph <- renderPlot({
    req(rv$centrality_results$closeness, rv$igraph)
    scores <- rv$centrality_results$closeness
    scores[is.na(scores)] <- 0
    build_ggraph_plot(rv$igraph, input, node_size_scores = scores)
  ...
  output$betweenness_plot <- renderVisNetwork({
    ...
    vis_data$nodes$value <- scales::rescale(scores, to = c(input$node_size * 0.4, input$node_size * 3))
    vis_data$nodes$title <- paste0("<b>", vis_data$nodes$label, "</b><br/>Betweenness: ", round(scores, 3))
  ...
  output$betweenness_ggraph <- renderPlot({
    req(rv$centrality_results$betweenness, rv$igraph)
    scores <- rv$centrality_results$betweenness
    build_ggraph_plot(rv$igraph, input, node_size_scores = scores)
  }, res = 110)
  ...
  output$eigenvector_plot <- renderVisNetwork({
    ...
    vis_data$nodes$value <- scales::rescale(scores, to = c(input$node_size * 0.4, input$node_size * 3))
    vis_data$nodes$title <- paste0("<b>", vis_data$nodes$label, "</b><br/>Eigenvector: ", round(scores, 3))
  ...
  output$eigenvector_ggraph <- renderPlot({
    req(rv$centrality_results$eigenvector, rv$igraph)
    scores <- rv$centrality_results$eigenvector
    build_ggraph_plot(rv$igraph, input, node_size_scores = scores)
  }, res = 110)
  ...
  # Top-5 summary: 4 columns side-by-side showing top nodes per measure
  output$centrality_top5_summary <- renderUI({
    ...
    node_names <- igraph::V(rv$igraph)$name %||%
                  as.character(seq_len(igraph::vcount(rv$igraph)))

    make_col <- function(scores, label) {
      n   <- min(5, sum(!is.na(scores)))
      idx <- order(scores, decreasing = TRUE, na.last = TRUE)[seq_len(n)]
      tags$div(
        tags$p(strong(label), style = "margin-bottom: 4px;"),
        tags$ol(style = "padding-left: 18px; margin: 0;",
          lapply(seq_len(n), function(i)
            tags$li(paste0(node_names[idx[i]], " (", round(scores[idx[i]], 3), ")"))
          )
        )
      )
    }
    ...
    fluidRow(
      column(3, make_col(rv$centrality_results$degree,      "⭐ Degree")),
      column(3, make_col(rv$centrality_results$closeness,   "🎯 Closeness")),
      column(3, make_col(rv$centrality_results$betweenness, "🌉 Betweenness")),
      column(3, make_col(rv$centrality_results$eigenvector, "👑 Eigenvector"))
    )
  })
