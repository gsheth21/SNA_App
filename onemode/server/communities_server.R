communities_server <- function(input, output, session, rv) {
  ...
  output$community_ggraph <- renderPlot({
    req(rv$community_results)
    membership <- igraph::membership(rv$community_results)
    palette    <- RColorBrewer::brewer.pal(max(max(membership), 3), "Set1")
    fill_cols  <- palette[membership]
