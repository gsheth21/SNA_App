bridges <- function(input, output, session, rv, g, components_result, vis_base) {
  ...
  output$bridges_ggraph <- renderPlot({
    req(bridges_result(), cutpoints_result())
    cutpoint_ids <- as.numeric(cutpoints_result())
    bridge_ids   <- as.numeric(bridges_result())
    n_nodes      <- igraph::vcount(g())
    n_edges      <- igraph::ecount(g())
    fill_cols    <- ifelse(seq_len(n_nodes) %in% cutpoint_ids, "#FF0000", "#CCCCCC")
    edge_cols    <- ifelse(seq_len(n_edges) %in% bridge_ids,   "#FF0000", "#888888")
    edge_widths  <- ifelse(seq_len(n_edges) %in% bridge_ids,   3, 0.8)
    build_ggraph_plot(g(), input,
      node_fill_override  = fill_cols,
      edge_color_override = edge_cols,
      edge_width_override = edge_widths)
  }, res = 110)
